...
<!--login page start -->
          <div class="login-form-container">
            ...
            <div class="login-form">
                ...
                <input type="password" title="Must contain at least one number and one uppercase and lowercase letter, and at least 8 or more characters" name="password" id="CustomerPassword"  placeholder="Password" required="">
              
              <div class="button-box">
                <div class="login-toggle-btn">
                  <input type="submit" class="section-button" onaction="..\index.php">Login In
                  
                  <a href="forgetpwd.php" id="RecoverPassword">Forgot your password?</a>
...
<!--        <div id="RecoverPasswordForm" style="display: none;" class="login">
          <form method="post" action="https://phuler.myshopify.com/account/recover" accept-charset="UTF-8"><input type="hidden" name="form_type" value="recover_customer_password" /><input type="hidden" name="utf8" value="✓" />
          ...
          <div class="login-form-container">
            ...
            <div class="login-form">
              <input type="email"  name="txtemail" id="RecoverEmail" placeholder="Email">
              <div class="button-box">
                <div class="login-toggle-btn">
                  <button type="submit" name="btnoptsend">Submit</button>
...
<?php
    ...
    if(!$c)
    {
       ...
       die("error while connecting"); 
        ...
        $fname=$_POST["customer[first_name]"];
        $mname=$_POST["customer[middle_name]"];
        $lname=$_POST["customer[last_name]"];
        $DOB=$_POST["customer[dob]"];
        $gender=$_POST["customer[gender]"];
        $mobileno=$_POST["customer[cno]"];
        $address=$_POST["customer[address]"];
        $city=$_POST["customer[city]"];
        $pincode=$_POST["customer[pin]"];
        $username=$_POST["customer[user_name]"];
        $email=$_POST["customer[email]"];
        
        $password=password_hash($_POST["customer[password]"],PASSWORD_DEFAULT);
...
<!--[if (gt IE 9)|!(IE)]><!--> <html class="no-js"> <!--<![endif]-->
    ...
    <!-- Mirrored from phuler.myshopify.com/account/register by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 17 Jul 2024 15:38:12 GMT -->
    <!-- Added by HTTrack --><meta http-equiv="content-type" content="text/html;charset=utf-8" /><!-- /Added by HTTrack -->
    <head>
        ...
        <!-- Title and description ================================================== -->
        <title>
            Create Account &ndash; Phuler - Flower Shop Shopify Theme
        </title>
        ...
        <script id="__st">var __st = {"a": 2155708489, "offset": -14400, "reqid": "07c13bf9-b120-4ee4-bd8f-9ed71f93b747-1721230504", "pageurl": "phuler.myshopify.com\/account\/register", "u": "52785266325f"};</script>
        <script>window.ShopifyPaypalV4VisibilityTracking = true;</script>
        <script id="captcha-bootstrap">!function () {
        'use strict';const t = 'contact', e = 'account', n = 'new_comment', o = t => t.map((([t, e]) => `form[action*='/${t}']:not([data-nocaptcha='true']) input[name='form_type'][value='${e}']`)).join(',');function c(t, e) {
            try {
        ...
        if ("sendBeacon" in navigator && "performance" in window) {
            var session_token = document.cookie.match(/_shopify_s=([^;]*)/);function handle_abandonment_event(e) {
                var entries = performance.getEntries().filter(function (entry) {
                    ...
                    window.abandonment_tracked = true;var currentMs = Date.now();var navigation_start = performance.timing.navigationStart;var payload = {shop_id: 2155708489, url: window.location.href, navigation_start, duration: currentMs - navigation_start, session_token: session_token && session_token.length === 2 ? session_token[1] : "", page_type: "customers/register"};window.navigator.sendBeacon("https://monorail-edge.shopifysvc.com/v1/produce", JSON.stringify({schema_id: "online_store_buyer_site_abandonment/1.1", payload: payload, metadata: {event_created_at_ms: currentMs, event_sent_at_ms: currentMs}}));
                }
            }
            window.addEventListener('pagehide', handle_abandonment_event);}}());</script>
        <script id="web-pixels-manager-setup">(function e(e, n, a, t, r) {
                var o = "function" == typeof BigInt && -1 !== BigInt.toString().indexOf("[native code]") ? "modern" : "legacy";window.Shopify = window.Shopify || {};var i = window.Shopify;i.analytics = i.analytics || {};var s = i.analytics;s.replayQueue = [], s.publish = function (e, n, a) {
                                ...
                                <div class="login-form-container">
                                    ...
                                    <div class="login-form">
                                        <form method="post" id="create_customer" accept-charset="UTF-8" data-login-with-shop-sign-up="true"><input type="hidden" name="form_type" value="create_customer" /><input type="hidden" name="utf8" value="✓" />
                                            ...
                                            <input type="text" name="customer[first_name]" id="FirstName" class="input-full" placeholder="First Name" pattern="^[a-z]*$" autocapitalize="words" title="Must contain character" autofocus required>

                                            <label for="MiddleName" class="hidden-label">Middle name</label>
                                            <input type="text" name="customer[middle_name]" id="MiddleName" class="input-full" placeholder="Middle Name" pattern="^[a-z]*$" title="Must contain character" autocapitalize="words" required>

                                            <label for="LastName" class="hidden-label">Last Name</label>
                                            <input type="text" name="customer[last_name]" id="LastName" class="input-full" placeholder="Last Name" pattern="^[a-z]*$" title="Must contain character" autocapitalize="words" required>

                                            <label class="hidden-label">Date of Birth</label>
                                            <input type="date" name="customer[dob]" class="input-full" placeholder="Enter date of birth" required>

                                            <label class="hidden-label">Gender</label>
                                            <select class="input-full" name="customer[gender]" style="border:1px solid buttonface" required>
                                            ...
                                            <input type="tel" name="customer[cno]" id="MobileNo" placeholder="Type your Mobile No" pattern="[0-9]{10}" maxlength="10" title="Must contain number" required>

                                            <label for="Address" class="hidden-label">Address</label>
                                            <textarea rows="2" name="customer[address]" cols="7" style="height: 40px;border-color: silver" placeholder="Enter address" required=""></textarea>
                                            <br><br>
                                            <label for="City" class="hidden-label">City</label>
                                            <input type="text" name="customer[city]" value="Surat" required>

                                            <label class="hidden-label">Pincode</label>
                                            <input type="text" name="customer[pin]" pattern="[0-9]{6}" maxlength="6" title="Must contain number" placeholder="Enter pincode" required>

                                            <label for="UserName" class="hidden-label">User Name</label>
                                            <input type="text" name="customer[user_name]" id="UserName" class="input-full" placeholder="User Name" pattern="^[a-z]*$" title="Must contain character" autocapitalize="words" autofocus required>
                                            ...
                                            <div class="button-box">
                                                <div class="login-toggle-btn">
                                                    <input type="submit" class="section-button" value="Create" name="btnsubmit">
                                                </div>
                                            </div>
        ...
        <div id="shopify-section-footer" class="shopify-section">
            <footer class="footer-area" id="section-footer">
                ...
                #section-footer .effect-snow {
                    background-image: url(register.php\/\/via.placeholder.com\/500x500);
    ...
    <!-- Mirrored from phuler.myshopify.com/account/register by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 17 Jul 2024 15:38:14 GMT -->
</html>-->
